...
import { Upload, UserPlus, MoreVertical, Eye, EyeOff, Plus, X } from "lucide-react";
...
const EditConference = () => {
      ...
      try {
        ...
        if (event) {
          ...
          if (event.social_links && Array.isArray(event.social_links)) {
            const links: SocialLinkDraft[] = event.social_links.map((l: any) => ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      updateData.social_links = socialLinksPayload;
      ...
      if (thumbnail || logo || speakers.some(s => s.avatar)) {
        ...
        Object.keys(updateData).forEach((key) => {
          if (key === "speakers") {
            ...
            formDataToSend.append("social_links_json", JSON.stringify(updateData.social_links));
  ...
  const handleStatusChange = async (newStatus: "published" | "waiting") => {
    if (!id) return;

    const published = newStatus === "published";
    const result = await safeRequest(() =>
      api.patch(`/organizer/events/${id}/publish`, { published })
    );
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem
                  ...
                  disabled={formData.status === "waiting"}
                >
                  <EyeOff className="w-4 h-4 mr-2" />
                  Riêng tư
                </DropdownMenuItem>
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div
                className={`border-2 border-dashed rounded-lg flex flex-col items-center justify-center py-12 text-center cursor-pointer hover:border-primary transition-colors aspect-video ${
