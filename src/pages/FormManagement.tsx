...
const FormManagement = () => {
  ...
  const getDefaultFieldDescription = (
    fieldLabel?: string,
    fieldType?: string
  ) => {
    ...
    if (
      label.includes("họ và tên") ||
      label.includes("ho va ten") ||
      label.includes("họ tên") ||
      label.includes("ho ten")
    ) {
  ...
  const [statistics, setStatistics] = useState<{ registered: number }>({
    registered: 0,
  });

  const formInfo = {
    displayStatus: isPublished ? "Đã xuất bản" : "Chưa xuất bản",
    maxRegistrations: eventData?.data?.capacity
      ? String(eventData.data.capacity)
      : "N/A",
    currentRegistrations: statistics.registered,
    contactEmail:
      eventData?.organizer?.email || eventData?.contact_email || "N/A",
  };
  ...
  const handleFieldDelete = (fieldId: string) => {
    setFields((prev) =>
      prev.filter((field) => !field.isDefault && field.id !== fieldId)
    );
  };
      ...
      prev.map((field) => {
        if (field.id === fieldId && field.type === "multipleChoice") {
          return {
            ...
            options: [
              ...(field.options || []),
              `Tùy chọn ${(field.options?.length || 0) + 1}`,
            ],
          };
        }
        return field;
  ...
  const handleMoveField = (index: number, direction: "up" | "down") => {
    ...
    if (
      ...
      return;
    if (!currentField.isDefault && targetField.isDefault && direction === "up")
      return;
  ...
  const saveForm = async (publish = false) => {
    ...
    await safeRequest(async () => {
      if (formId) {
        ...
        const updated: any = await api.put(
          `/organizer/events/forms/${formId}/with-fields`,
          payload
        );
        if (updated) {
          if (publish) setIsPublished(true);
          // Reload fields to get updated IDs for new fields
          const form: any = await api.get(
            `/organizer/events/forms/event/${id}`
          );
          if (form && form.fields) {
            const mapped = (form.fields || []).map((f: any, idx: number) => {
              ...
              const fieldDescription =
                f.field_description &&
                String(f.field_description).trim().length > 0
                  ? f.field_description
                  : f.is_primary_key
                  ? getDefaultFieldDescription(f.field_label, f.field_type)
                  : "";
              return {
                id: f._id || String(idx),
                label: f.field_label,
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      // Load event data
      const event: any = await safeRequest(() =>
      ...
      // Load form data
      const form: any = await safeRequest(() =>
        api.get(`/organizer/events/forms/event/${id}`)
      );
      if (form) {
        setFormId(form._id);
        ...
        const mapped = (form.fields || []).map((f: any, idx: number) => {
          ...
          const fieldDescription =
            f.field_description && String(f.field_description).trim().length > 0
              ? f.field_description
              : f.is_primary_key
              ? getDefaultFieldDescription(f.field_label, f.field_type)
              : "";
        ...
        if (form.is_public) {
          const stRes: any = await safeRequest(() =>
                        ...
                        <div className="flex-auto">
                          ...
                          {field.type === "textarea" ? (
                            ...
                            <div className="border-2 border-dashed rounded-lg p-4 text-center text-muted-foreground">
                              ...
                              <p className="text-sm">
                                Người dùng sẽ tải tệp tin tại đây
                              </p>
                            </div>
                          ) : (
                            <Input
                    ...
                    <Button
                      ...
                      onClick={() => {
                        setNewField((prev) => ({
                          ...
                          options: [
                            ...(prev.options || []),
                            `Tùy chọn ${(prev.options?.length || 0) + 1}`,
                          ],
                        }));
                      }}
                    >
