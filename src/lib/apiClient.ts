...
export class ApiClient {
	...
	async request<TResponse = unknown, TBody = unknown>(path: string, opts: RequestOptions<TBody> = {}): Promise<TResponse> {
		...
		try {
			...
			throw new ApiError({ status: 0, message: (err as Error).message || "Network error" });
		} finally {
...
export const api = new ApiClient({
	...
	getAuthToken: () => {
		...
		return token && token !== "undefined" && token !== "null" && token.trim() !== "" ? token : undefined;
	},
